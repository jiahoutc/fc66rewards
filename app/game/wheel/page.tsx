...
export default function WheelGame() {
    ...
    const spinWheel = async () => {
        if (spinning || result || !user || user.credits <= 0) return
        ...
        const data = await res.json()
        const wonReward = data.reward
        setUser((prevUser: any) => ({ ...prevUser, credits: prevUser.credits - 1, isClaimed: true, assignedReward: wonReward.name }));
        ...
        <div className="min-h-screen bg-zinc-900 flex flex-col items-center justify-center overflow-hidden relative">
            ...
            <div className="relative w-[350px] h-[350px] md:w-[500px] md:h-[500px]">
                ...
                >
                        ...
                        {/* Spin Button (Center) */}
                        {/* The original spin button was here. It is now moved outside the wheel container for better layout and replaced with the new logic below. */}
                    </div>
                    ...
                    {!spinning && !result && (
                        <div className="mt-8 text-center z-10">
                            {user.credits > 0 ? (
                                <button
                                    onClick={spinWheel}
                                    className="px-12 py-4 bg-gradient-to-r from-gold to-yellow-600 text-black font-bold text-2xl rounded-full shadow-[0_0_30px_rgba(255,215,0,0.4)] hover:scale-105 transition-transform border-2 border-white"
                                >
                                    SPIN ({user.credits} Left)
                                </button>
                            ) : (
